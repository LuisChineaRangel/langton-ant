...
/// @brief SmartVector Class Header
#pragma once
...
/// @param begin First legal vector index
/// @param end Vector end (first illegal index)
template <class T>
...
template <class T>
size_t SmartVector<T>::Begin(void) const {
  return begin_;
}
...
template <class T>
size_t SmartVector<T>::Center(void) const {
...
template <class T>
size_t SmartVector<T>::End(void) const {
  return end_;
}
...
/// @return Void
template <class T>
void SmartVector<T>::Resize(size_t begin, size_t end) {
  assert((end - begin) >= 0);
...
/// @param pos Index of value to return
/// @return Value located in the index
template <class T>
T &SmartVector<T>::operator[](size_t pos) {
  if ((pos < begin_) || (pos >= end_)) throw WrongIndex();
...
/// @return Value located in the index
template <class T>
T &SmartVector<T>::operator[](size_t pos) const {
  if ((pos < begin_) || (pos >= end_)) throw WrongIndex();
